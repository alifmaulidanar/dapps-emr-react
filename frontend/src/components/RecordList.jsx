...
export default function RecordList({ recordItems, chosenPatient, appointmentData }) {
  ...
  const handleNavigate = (record) => {
    const matchedAppointment = appointmentData.find(appointment => appointment.data.appointmentId === record.appointmentId)?.data;
    ...
    <div className="grid gap-4">
      {recordItems.length > 0 ? (
        recordItems.map((record, index) => (
          <div key={index} onClick={() => handleNavigate(record)} className="cursor-pointer" >
            <div className="w-full p-6 bg-white border border-gray-200 rounded-lg shadow">
              <div className="grid items-center justify-between grid-cols-4 mb-4">
                <div className="flex col-span-3 flex-nowrap gap-x-4">
                  <h5 className="font-bold text-gray-900">{record.judulRekamMedis}</h5>
