...
export default function NursePelayananMedis({ role }) {
  ...
  const saveDataToSessionStorage = (emrNumber) => {
    ...
    window.location.assign("/nurse/pelayanan-medis/patient-details");
  };
  ...
  return (
    <>
      ...
      <Modal width={800} open={isModalOpen} onCancel={handleCancel} footer={null}>
        {selectedAppointment && (
          <PatientAppointmentDisplayStaff data={{appointment: {data: selectedAppointment}}} token={token} prerole="nurse" />
        )}
      </Modal>
    </>
  );
